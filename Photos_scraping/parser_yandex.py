...
class Parser():
    ...
    # scrolling down until no updates on page or get enough tags===================================================================================
    ...
    def parse_photos(self, row):
        ...
        if (len(photos_divs) > 0):
            for photo in photos_divs:
                ...
                mid_photos_organization_pd.loc[len(mid_photos_organization_pd)] = [row['original_address'], row['found_address'], row['place_name'], row['place_link'], row['coords'], photo_link, isinside, row['ya_id'], photos_number]
    ...
    # saves all files collected during session=======================================================================================================
    ...
    # download panorama by link to panorama to file with filename====================================================================================        
...
# Creates one thread for parse_photos_orgs_threads function==========================================================================================
...
# download photos links from organization pages in multiple threads==================================================================================    
...
#combines multiple files with gathered data in one file, deletes duplicates and errors===============================================================
...
# Creates one thread for parse_organization_threads function=========================================================================================
...
#gather all building links and organization links from addresses in multiple threads=================================================================
def parse_organizations_threads(diap, threads_num):
    ...
    slice_len=int((diap[1]-diap[0])/threads_num)
    threads = [
        Thread(target=parse_orgs_by_address, args=(streets_pd[i*slice_len:(i+1)*slice_len], diap, i,))
        for i in range(threads_num)
    ]
...
#update rows in file with process statuses===========================================================================================================    
...
# delete all temporary files with saved data=========================================================================================================
...
#after gathering all organizations and buildings links separates links to organizations only=========================================================
...
# creates one thread for download_photos_threads function============================================================================================
...
#downloads photos by gathered links in multiple threads==============================================================================================        
...
# creates one thread for download_panoramas_threads function============================================================================================      
...
#download panoramas by gathered links in multiple threads============================================================================================
...
#combine all photos data in a single file============================================================================================================          
...
if __name__ == '__main__':
    parse_organizations_threads([0,60], 2)      # gather all organization links and building links from addresses in file on rows from 0 to 60 in 2 threads
    parse_photos_orgs_threads([0, 5000], 5)     # gather all photos links from organization pages on organization links in file on rows from 0 to 5000 in 5 threads
    clear_saved_files('Иркутск_rows_0-60')      # delete all temporary files with data which name contains 'Иркутск_rows_0-60'
    rebuild_stat_file(org_photos=True)          # update process statuses in status file
    download_photos_threads('gathered_data/yandex/results/all_photos_all.csv', 0, 5)        #download photos by links in file from 0-th row in 5 threads
    download_panoramas_threads('gathered_data/yandex/results/all_photos_all.csv', 0, 1)     #download panoramas by links in file form 0-th row in 1 thread
    
    
    
    combine_files('gathered_data/yandex/organizations', ['place_link'])         #combines all files with links on organizations
    combine_files('gathered_data/yandex/building_photos', ['photo_link'])       #combines all files with links on building photos
    combine_files('gathered_data/yandex/organization_photos', ['photo_link'])   #combines all files with links on organization photos
    combine_files('gathered_data/yandex/results', ['photo_link'])               #combines all files with links on building
    make_stat_org_file()                                                        #after gathering all organizations and buildings links separates links to organizations only
    
    build_photos_links()                        #combine all photos data in a huge single file
